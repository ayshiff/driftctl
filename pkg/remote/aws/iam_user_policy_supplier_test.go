...
func TestIamUserPolicySupplier_Resources(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllUserPolicies").Return([]string{}, nil)
			},
			err: nil,
		},
		{
			...
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllUserPolicies").Return([]string{
					*aws.String("loadbalancer3:test3"),
					*aws.String("loadbalancer2:test22"),
					*aws.String("loadbalancer2:test32"),
		...
		},
		{
			...
			dirName: "iam_user_policy_empty",
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllUserPolicies").Return(nil, awserr.NewRequestFailure(nil, 403, ""))
			},
